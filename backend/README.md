...
Schedules a post. Use `application/json` for text or `multipart/form-data` when uploading media. The upload field name is `media`.
...
- Scheduled media is uploaded to Zernio through its presigned upload flow before the post is created. Generated images are still stored in Cloudinary under the `ai-generations` folder.
